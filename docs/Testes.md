...
A captura da execução de `PlataformaGamificacaoTest` pelo Test Runner for Java mostra **7 testes executados, sem falhas, erros ou testes ignorados**. Entre os cenários estão a média exatamente 7,0, a reprovação por nota inferior a 7,0, o curso não concluído, a nova tentativa aprovada e o acúmulo após uma segunda aprovação.

![Sete testes de domínio aprovados no JUnit](evidencias/Test_JUnit.png)

## Maven — suíte completa

A captura do Maven mostra **19 testes executados, sem falhas, erros ou testes ignorados**, com `BUILD SUCCESS`. Esse total inclui os testes de domínio, API, serviço, persistência H2 e OpenAPI.
